...
} from './dtos/get-all-podcasts.dto';
...
export class PodcastsService {
  ...
  async getAllPodcasts({
    ...
    try {
      let podcastQuery = await this.podcastRepository.createQueryBuilder(
        'podcast',
      );
      if (categoryName) {
        podcastQuery = podcastQuery.where('podcast.category = :categoryName', {
          categoryName,
        });
      }
      const clonedPodcastQuery = podcastQuery.clone();
      const podcasts = await podcastQuery
        .addSelect('COUNT(subscribers.id)', 'podcast_totalSubscribers')
        .leftJoin('podcast.subscribers', 'subscribers')
        .groupBy('podcast.id')
        .offset((page - 1) * this.PageSize)
        .limit(this.PageSize)
        .getMany();
      const { totalResults } = (await clonedPodcastQuery
        .select('COUNT(podcast.id)', 'totalResults')
        .getRawOne()) || { totalResults: 0 };
      return {
        ok: true,
        podcasts,
        totalPages: this.getTotalPages(totalResults),
        totalResults,
      };
    } catch {
  ...
  async createEpisode(
    user: User,
    { podcastId, title, category, audioURL }: CreateEpisodeInput,
